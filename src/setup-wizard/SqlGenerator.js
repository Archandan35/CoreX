...
export class SqlGenerator {
  ...
  _genUserTrigger() {
    ...
    return [
      ...
      // on behalf of the anon/authenticated caller. SET search_path is mandatory
      // for any SECURITY DEFINER function to avoid search-path hijacking and to
      // guarantee `public.users` resolves deterministically.
      ...
      '    -- server-side (this is the security-critical guarantee and must NOT',
      '    -- depend on client-supplied metadata, which can be tampered with).',
      '    -- Subsequent accounts honour the metadata flag, defaulting to false.',
      '    CASE',
      ...
      '    END,',
      '    -- permissions is a TEXT[] column. The metadata value arrives as a',
      '    -- JSON array string, so it must be unpacked into a native text[] via',
      '    -- jsonb_array_elements_text — casting jsonb directly to text[] is not',
      '    -- an implicit Postgres cast and would raise a type error here, which',
      '    -- (because this is an AFTER INSERT trigger) would roll back the',
      '    -- auth.users insert and leave the user with no profile record.',
      "    COALESCE(",
