...
export const roleLogic = {
  ...
  // Resolve the admin role by authority (all === true), never by name/code string.
  async getSuperAdminRoleCode() {
    const roles = await roleService.list();
    return roles.find((r) => r.all === true)?.code || null;
  },
  ...
  async bulkRemove(ids, actor) {
    try {
      ...
      await auditService.record({ action: 'role.bulkDelete', module: 'roles', user: actor, details: `Deleted ${deleted} role(s); skipped ${skippedInUse} in-use; failed ${failed}` });
