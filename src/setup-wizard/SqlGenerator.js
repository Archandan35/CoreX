...
/**
 * SqlGenerator — single source of truth for all SQL output.
 *
 * Provides a SINGLE `generate(report, options)` method:
 *
 *   - `options.full === true`  → emit the COMPLETE canonical schema
 *   - `options.full === false` → emit only DELTA (objects the report says are missing)
 *
 * Because the report is used BOTH ways, the preview and download are
 * generated by the same code — they cannot diverge.
 */
export class SqlGenerator {
  ...
  _genHelperFunctions(report, full) {
    ...
    // exec_sql — always emitted in full mode; in delta mode emitted when
    // the function itself is missing from the database.
    const execSqlMissing = full || missing.some(
      (i) => i.name === 'exec_sql' || (i.type === 'function' && i.name === 'exec_sql')
    );
    if (execSqlMissing) blocks.push(buildExecSqlFunction());

    // check_admin_exists — always emitted in full mode; in delta mode
    // emitted when the function itself is missing.
    const checkAdminMissing = full || missing.some(
    ...
    //   - the function itself is missing from the database, OR
    //   - any users-table RLS policy is missing (the policies reference
    //     is_admin_user(), so the function must exist first), OR
    //   - the users table itself is missing (RLS policies referencing
    //     is_admin_user() will be generated by _genMissingRLS, so the
    //     function must exist before those policy DO blocks execute).
